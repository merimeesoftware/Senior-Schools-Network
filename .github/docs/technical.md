...
- **Philosophical Alignment**: Technical choices support poetic knowledge (e.g., immersive, Markdown-rendered excerpts for sensory integration) and Catholic exclusivity (e.g., content presentation grounded in faith-based themes from uploaded documents). Support narrative flows with gymnasium emphasis (e.g., interactive adventure resources, downloadable guides) and scientific ties (e.g., modular excerpts on poetic foundations for STEM).
- **User Flow Support**: Architecture enables enhanced user journeys—from school consideration to personal application (e.g., homeschool adaptations) to founding inspiration—through component reusability and accessible design.
...
- **Open-Source Focus**: Prioritize free, open-source tools to align with humane, distributist principles (per Chesterton's _The Outline of Sanity_), avoiding vendor lock-in and promoting community-driven development.
...
- **Component-Based Design**: Reusable, modular components (e.g., QuoteCard for integrating excerpts from Tolkien's _Mythopoeia_ or Aquinas's _Summa_, including adventure quotes for gymnasium emphasis; BookTable for rendering staged lists from Senior's resources; InteractiveMap for school directories with stage filters like gymnasium focus; DownloadCard for founding toolkits and home guides). This promotes organic growth, allowing easy additions like media embeds from resources.md.
...
- **Accessibility Features**: Semantic HTML; ARIA attributes on interactive elements (e.g., expandable excerpts, founding toolkit checklists); color contrast compliant with Tailwind; alt text for images derived from document contexts (e.g., "Illustration of poetic knowledge from Hugh of St. Victor," "Adventure scene for gymnasium stage"). Support for enhanced user flows like personal application guides and founding resources.
...
- **Workflow Structure** (.github/workflows/ci.yml):
  ...
  - **Jobs**:
    - Lint & typecheck: ESLint + `tsc --noEmit` via Bun.
...
- **Monitoring & Maintenance**: Cloudflare Workers observability is enabled in `wrangler.jsonc`; manual audits for philosophical fidelity (e.g., ensure content evokes wonder per excerpts.md).
